...
class SearchFoodScreen extends StatefulWidget {
  const SearchFoodScreen({Key key, this.user}) : super(key: key);
...
class _SearchFoodScreenState extends State<SearchFoodScreen> {
  ...
  void _searchFood() async {
    ...
    logger.d("user", widget.user.uid);

    FoodList foodList = await ApiService.instance.generateFoodList(
      query: "",
    );
    ...
    Navigator.push(
        context,
        MaterialPageRoute(
          builder: (_) => FoodScreen(foodList: foodList, user: widget.user),
        ));
  }
